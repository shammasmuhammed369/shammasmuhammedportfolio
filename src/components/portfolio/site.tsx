...
export function DetailGrid({ items }: {items: {label:string; value:ReactNode}[]}) { return <dl className="grid gap-px overflow-hidden border border-border bg-background sm:grid-cols-2"><>{items.map((x,i)=><div key={x.label} className="border border-border bg-panel p-5"><dt className="font-mono text-[0.63rem] uppercase tracking-[0.16em] text-primary">{String(i+1).padStart(2,"0")} / {x.label}</dt><dd className="mt-3 text-sm leading-6 text-muted-foreground">{x.value}</dd></div>)}</></dl> }
export function TopicGrid({items}:{items:string[]}) { return <div className="grid gap-px border border-border bg-background sm:grid-cols-2 lg:grid-cols-3">{items.map((x,i)=><div key={x} className="card-lift border border-border bg-panel p-5"><span className="font-mono text-[0.65rem] text-primary">{String(i+1).padStart(2,"0")}</span><p className="mt-4 text-sm font-medium">{x}</p></div>)}</div> }
